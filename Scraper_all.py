import pandas as pd
from page_utils import CCGPScraperall
...
def main(start_page, end_page, wait1=1, wait2=3):
    ...
    biaoshu = pd.DataFrame({}, columns=['标题', '项目概况', '一', '二', '三', '四', '五', '六', '七', '八', '九',
                                        '附件链接', '链接'])
    ...
    for i in range(1, len(ccgp_content)+1):
        try:
            ...
            biaoshu.loc[i] = [title, summary, tag1_text, tag2_text, tag3_text, tag4_text, tag5_text, tag6_text,
                              tag7_text, tag8_text, tag9_text, links, all_urls[i]]
            print('第', i, '条处理完成')
